##
# audio_tf_taller.py
# Author: Luisa Andrade, Jessica Castillo, y Juan David García.
# Date: 2020
# Taller de identificación de pistas de audio
# a partir de la transformada Tiempo-Frecuencia

# Importamos las librerías numpy y matplotlib.pyplot
import numpy as np
...
def get_hash(FsHz,x1):
    ...
    for FreqIter in tqdm(range(np.size(FreqTestHz)),desc='FreqTest'):
    ...
    for i in tqdm(range(np.size(TFMatAbsMax, 0)),desc='Max. TF'):
    ...
    for i in tqdm(range(np.size(TFMatAbsMax, 0)),'Max. patch'):
        for j in range(np.size(TFMatAbsMax, 1)):
    ...
    for i in tqdm(range(np.size(TFMatAbsMaxBin, 0)),desc='Extracting hashes'):
    ...
    return HashValues, TFMatAbs, FreqTestHz
